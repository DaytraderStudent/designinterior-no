...
import { Star, ThumbsUp, ThumbsDown, CheckCircle, ExternalLink, Ruler, Tag, Palette, Home } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${product.name} fra ${product.brand} — Anmeldelse og pris ${formatNOK(product.price)} | designinteriør.no`,
    description: `Les vår ærlige anmeldelse av ${product.name} fra ${product.brand}. ${product.review_summary} Pris: ${formatNOK(product.price)}.`,
    openGraph: {
      title: `${product.name} fra ${product.brand} — ${product.rating}/5 | ${formatNOK(product.price)}`,
...
export default async function ProductPage({ params }: Props) {
  ...
  // JSON-LD with Review structured data
  const jsonLd = {
                    ...
                    <Card className="overflow-hidden group hover:shadow-md transition-all">
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between mt-1">
                          <p className="font-mono text-sm font-bold text-accent">{formatNOK(p.price)}</p>
